...
export function ResizeImageDialog({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      resizePng(sourceBlob, currentDpi, targetDpi, widthPx, heightPx)
        .then(({ blob }) => {
  ...
  }, [sourceBlob, currentDpi, targetDpi, widthPx, heightPx]);
  ...
  async function handleApply() {
    ...
    setError(null);
    try {
      // Re-run at the exact current targetDpi rather than trusting a
      // possibly-still-debouncing previewBlob — cheap, and removes any
      // chance of applying a stale preview.
      const { blob: finalBlob } = await resizePng(sourceBlob, currentDpi, targetDpi, widthPx, heightPx);
  ...
  return (
    <div className={styles.overlay} role="presentation">
      <div className={styles.card} role="dialog" aria-modal="true" aria-label="Reduce image file size">
        ...
        <p className={styles.desc}>Resamples "{fileName}" to a lower resolution, keeping its physical print size.</p>
        ...
        <div className={styles.grid}>
          ...
          <div className={styles.label}>Expected size</div>
          <div className={styles.value}>
